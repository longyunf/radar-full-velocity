'''
Prepare image pairs (key frame and the next one) for optical flow
'''
...
if __name__ == '__main__':
    ...
    for sample_idx in sample_indices:
        ...
        if cam_data['next']:
            ...
            im = downsample_im(im1, downsample_scale, y_cutoff)
            im_next = downsample_im(im2, downsample_scale, y_cutoff)
                        
            io.imsave(join(dir_data_out, '%05d_im_full.jpg' % sample_idx), im1)
            io.imsave(join(dir_data_out, '%05d_im_full_next.jpg' % sample_idx), im2)
