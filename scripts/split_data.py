...
import _init_paths
from gt_velocity import gt_box_key
...
if __name__ == '__main__':   
    ...
    trainval_scenes = defaultdict(list)          
    test_scenes = defaultdict(list)        
    for scene in tqdm(nusc.scene):
        if 'rain'.lower() in scene['description'].lower() or 'Night'.lower() in scene['description'].lower():
            continue
        name = scene['name'] 
        if name in SCENE_SPLITS['train']:
            trainval_scenes['names'].append(name)
            trainval_scenes['tokens'].append(scene['token'])   
        elif name in SCENE_SPLITS['val']:
            test_scenes['names'].append(name)
            test_scenes['tokens'].append(scene['token'])
            
            
    trainval_scenes['names'], trainval_scenes['tokens'] = np.array(trainval_scenes['names']), np.array(trainval_scenes['tokens'])    
    N_trainval = len(trainval_scenes['names'])
    indices = np.arange(N_trainval)
    np.random.shuffle(indices)
    trainval_scenes['names'] = trainval_scenes['names'][indices]
    trainval_scenes['tokens'] = trainval_scenes['tokens'][indices] 
    
       
    n_train_scenes = int(round(N_trainval * train_ratio))
    n_val_scenes = N_trainval - n_train_scenes
      
    train_scenes = defaultdict(list)
    val_scenes = defaultdict(list)
    
    train_scenes['names'] = trainval_scenes['names'][:n_train_scenes]
    train_scenes['tokens'] = trainval_scenes['tokens'][:n_train_scenes]
    val_scenes['names'] = trainval_scenes['names'][n_train_scenes:]
    val_scenes['tokens'] = trainval_scenes['tokens'][n_train_scenes:]
           
    train_sample_idx = []
    val_sample_idx = []
    test_sample_idx = []
            
    for idx, sample in tqdm(enumerate(nusc.sample)):        
        if is_first_2_sample_in_scene(idx) or is_last_2_sample_in_scene(idx):
            continue        
        if sample['scene_token'] in train_scenes['tokens']:
            train_sample_idx.append(idx)
        elif sample['scene_token'] in val_scenes['tokens']:
            val_sample_idx.append(idx)
        elif sample['scene_token'] in test_scenes['tokens']:
    ...
    sample_split = {'all_indices': all_idx,
    ...
    for sample_idx in tqdm(sample_split['all_indices']):
        gt = gt_box_key(nusc, sample_idx)
        is_valid_frame = gt.v_label_exist(thres_n_gt_pts=2)   # ignore frames having fewer than 2 radar points with GT velocity
        if is_valid_frame:
            if sample_idx in sample_split['train_sample_indices']:
                train_sample_idx.append(sample_idx)
            elif sample_idx in sample_split['val_sample_indices']:
                val_sample_idx.append(sample_idx)
            elif sample_idx in sample_split['test_sample_indices']:
                test_sample_idx.append(sample_idx)
                
    
    all_idx = train_sample_idx + val_sample_idx + test_sample_idx
    
    sample_split_new = {'all_indices': all_idx,
                        'train_sample_indices': train_sample_idx,
                        'val_sample_indices': val_sample_idx,
                        'test_sample_indices':  test_sample_idx }
    
    
    print('train: %d, val: %d, test: %d' % ( len(train_sample_idx), len(val_sample_idx), len(test_sample_idx) ) )
    torch.save(sample_split_new, join(args.dir_data, 'sample_split.tar'))
